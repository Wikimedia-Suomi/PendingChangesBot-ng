...
class ReferenceOnlyEditTests(TestCase):
    ...
    def test_replacing_reference(self):
        ...
        # Unnamed refs matched by position when counts match and content unchanged
        self.assertTrue(result["is_reference_only"])
        self.assertEqual(len(result["added_refs"]), 0)
        self.assertEqual(len(result["modified_refs"]), 1)
        self.assertEqual(len(result["removed_refs"]), 0)
...
class URLExtractionTests(TestCase):
    ...
    def test_extract_domain_from_url(self):
        ...
        self.assertEqual(_extract_domain_from_url("https://example.com/page"), "example.com")
        self.assertEqual(_extract_domain_from_url("http://test.org"), "test.org")
        self.assertEqual(_extract_domain_from_url("https://sub.domain.com/path"), "sub.domain.com")

    def test_extract_domain_with_port(self):
        """Extract domain from URL with port."""
        self.assertEqual(_extract_domain_from_url("http://example.com:8080/page"), "example.com")
...
class DomainVerificationTests(TestCase):
    ...
    def test_check_domain_exists_returns_true(self, mock_site_class):
        ...
        mock_site = mock.Mock()
        mock_site.exturlusage.return_value = iter(["page1", "page2"])
        mock_site_class.return_value = mock_site

        result = _check_domain_exists_on_wiki("example.com", self.wiki)

        self.assertTrue(result)
        mock_site.exturlusage.assert_called_once_with("example.com", namespaces=[0], total=2)
    ...
    @mock.patch("reviews.autoreview.pywikibot.Site")
    def test_check_domain_only_self_match_returns_false(self, mock_site_class):
        """Domain with only 1 match (itself) returns False."""
        mock_site = mock.Mock()
        mock_site.exturlusage.return_value = iter(["page1"])
        mock_site_class.return_value = mock_site

        result = _check_domain_exists_on_wiki("newdomain.com", self.wiki)
