...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            type=str,
            default='',
            help='Prefix to add to approval comments'
        )
        parser.add_argument(
            '--preview-only',
            action='store_true',
            help='Only preview comments without processing'
        )
        parser.add_argument(
            '--verbose',
            action='store_true',
    ...
    def _run_scenario(self, scenario: str, comment_prefix: str, preview_only: bool, verbose: bool):
        ...
        if preview_only:
            ...
            self.stdout.write(f'Preview mode: {result["message"]}')
        else:
        ...
        self.stdout.write(f'Statistics: {stats["approved_count"]}/{stats["total_revisions"]} approved ({stats["approval_rate"]:.1f}%)')
    ...
    def _show_usage_examples(self):
        """Show usage examples."""
        self.stdout.write('\nUsage Examples:')
        self.stdout.write('  python manage.py test_approval_comment --scenario mixed --preview-only')
        self.stdout.write('  python manage.py test_approval_comment --scenario bot --comment-prefix "Auto: "')
