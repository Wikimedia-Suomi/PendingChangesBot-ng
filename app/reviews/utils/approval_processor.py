...
def process_and_approve_revisions(
    ...
    try:
        ...
        approved_count = len([r for r in autoreview_results if r.get("decision", {}).get("status") == "approve"])
...
def get_approval_statistics(autoreview_results: List[Dict]) -> Dict:
    ...
    try:
        ...
        approved_revisions = [r for r in autoreview_results if r.get("decision", {}).get("status") == "approve"]
